...
app
    ...
    .controller('UserAccountController', ['$scope', '$http', '$mdMedia', 'userApi', '$filter', 'payDialog', 'qrcodeDialog', '$interval', '$localStorage', 'changePasswordDialog', 'payByGiftCardDialog', 'subscribeDialog', '$q', '$state', 'wireGuardConfigDialog',
        ($scope, $http, $mdMedia, userApi, $filter, payDialog, qrcodeDialog, $interval, $localStorage, changePasswordDialog, payByGiftCardDialog, subscribeDialog, $q, $state, wireGuardConfigDialog) => {
                ...
                account.forEach(a => {
                    ...
                    if (a.server && $scope.config.showAllServer) {
                        a.serverList.sort((f, e) => {
                            if (a.server.indexOf(f.id) >= 0 && a.server.indexOf(e.id) < 0) {
                                return -1;
                            }
                            return 1;
                        });
            ...
            $scope.setInterval($interval(() => {
                if (Date.now() - $localStorage.user.accountInfo.time <= 15 * 1000) {
                    return;
                }
                getUserAccountInfo();
            ...
            $scope.isSS = server => {
                return (server && server.type === 'Shadowsocks');
            };
            $scope.isTJ = server => {
                return (server && server.type === 'Trojan');
            };
        ...
        ($scope, $state, userApi, alertDialog, $http, $localStorage) => {
            ...
            $scope.confirm = () => {
                ...
                userApi.changePassword($scope.data.password, $scope.data.newPassword).then(success => {
                    alertDialog.show('Пароль успешно изменен, пожалуйста, войдите снова', 'Ок')
                        .then(() => {
    ...
    .controller('UserTelegramController', ['$scope', '$http', '$interval',
        ($scope, $http, $interval) => {
            $scope.setTitle('Привязка Telegram');
            $scope.setMenuButton('arrow_back', 'user.settings');
    ...
    .controller('UserRefController', ['$scope', '$http', '$filter',
        ($scope, $http, $filter) => {
            ...
            $scope.setMenuButton('arrow_back', 'user.settings');
            $http.get('/api/user/ref/code').then(success => {
                $scope.code = success.data;
            });
            $http.get('/api/user/ref/user').then(success => {
                $scope.user = success.data;
            });
            $scope.getRefUrl = code => `${$scope.config.site}/home/ref/${code}`;
            $scope.clipboardSuccess = event => {
                $scope.toast($filter('translate')('Ссылка на приглашение скопирована в буфер обмена.'));
            };
        }
    ])
