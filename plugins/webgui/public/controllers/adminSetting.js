...
app.controller('AdminSettingsController', ['$scope', '$state',
    ...
    ($scope, $http, $timeout, $state, adminApi, alertDialog, $localStorage) => {
        ...
        $scope.confirm = () => {
            ...
            adminApi.changePassword($scope.data.password, $scope.data.newPassword).then(success => {
                alertDialog.show('修改密码成功，请重新登录', 'Ок')
                    .then(() => {
    ...
    ($scope, $http, $timeout, $interval, $state, $stateParams) => {
        ...
        $http.get('/api/admin/setting/payment').then(success => {
            ...
            if (!$scope.paymentData.refTime) {
                $scope.paymentData.refTime = '0h';
            }
    ...
    ($scope, $http, $timeout, $state, $mdMedia) => {
        ...
        $scope.getCode = search => {
            ...
            $http.get('/api/admin/setting/ref/code', {
                ...
                if ($state.current.name !== 'admin.refCodeList') {
                    return;
                }
                $timeout(() => {
                    $scope.getCode(search);
        ...
        $scope.view = inview => {
            if (!inview || $scope.isCodeLoading || $scope.isCodePageFinish) {
                return;
            }
            $scope.getCode();
        };
    ...
    ($scope, $http, $timeout, $state, $mdMedia) => {
        ...
        $scope.getUser = () => {
            ...
            $http.get('/api/admin/setting/ref/user', {
                ...
                if ($state.current.name !== 'admin.refUserList') {
                    return;
                }
                $timeout(() => {
                    $scope.getUser();
    ...
    .controller('AdminMyRefCodeController', ['$scope', '$http', '$filter', '$state', '$mdMedia',
        ($scope, $http, $filter, $state, $mdMedia) => {
            ...
            $http.get('/api/admin/ref/code').then(success => {
                $scope.code = success.data;
            });
            $http.get('/api/admin/ref/user').then(success => {
        ...
        ($scope, $http, $timeout, $state, $mdMedia, alertDialog) => {
            ...
            const getRefCode = userId => {
                $http.get(`/api/admin/ref/code/${userId}`).then(success => {
                    $scope.refCode = success.data;
                    if ($scope.refCode.length) {
                        $scope.sourceUserCode = $scope.refCode[0].code;
                    }
                });
            };
            $scope.sourceUser = {
