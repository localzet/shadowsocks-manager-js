                ...
                confirmDialog.show({
                    ...
                    fn: function () {
                        return $http.delete(`/api/admin/order/${$scope.orderId}`).catch(err => {
                            if (err.status === 403) {
                                let errData = '删除订单失败';
                                if (err.data === 'account with this order exists') {
                                    errData = '无法删除订单，请先删除订单对应的账号';
                                }
                                if (err.data === 'giftcard with this order exists') {
                                    errData = '无法删除订单，请先删除订单对应的充值码';
                                }
                                if (err.data === 'flowpack order exists') {
                                    errData = '无法删除订单，请先删除对应的流量包订单';
                                }
